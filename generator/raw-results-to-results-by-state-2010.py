...
# Converts the raw election results to results by state. Works for 2000-2010
...
with open(sys.argv[1], "rU") as csvfile:
    ...
    for row in reader:
        ...
        if len(state) > 0 and len(district) > 0 and len(votes) > 0 and len(party) > 0 and includes(state) and district.isdigit(): #Make sure row is actually for a general election candidate
            ...
            if votes == "Unopposed" or votes == "Unopposed   " or votes == "#":
                votes = -1

            if votes == "n/a" or votes == "??":
                votes = 0

            if party == "R" or party == "REP":
                rep += int(votes)
            elif party == "D" or party == "DFL" or party == "DEM":
                dem += int(votes)
